...
fn invalid_json_returns_parse_error() {
    ...
    let tendrils_override_json = &temp.path().join("tendrils-override.json");
    std::fs::write(&tendrils_override_json, "I'm not JSON").unwrap();

    let actual = get_tendril_overrides(&temp.path());
...
fn valid_json_returns_tendrils() {
    ...
    let tendrils_json = &temp.path().join("tendrils-override.json");
