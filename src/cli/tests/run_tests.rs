...
fn tendril_action_tendrils_are_filtered_by_profile(
    ...
    assert_eq!(writer.all_output_lines()[0], "No local overrides were found.");
    assert!(writer.all_output_lines()[4].contains("misc2.txt"));
    assert!(writer.all_output_lines()[4].contains("NotFound"));
    assert!(writer.all_output_lines()[6].contains("misc3.txt"));
    assert!(writer.all_output_lines()[6].contains("NotFound"));
}
