...
def main [formatted_mdata_path: string] {
    ...
    print $"<!-- This file is auto-generated using 3rd-party-compile-licenses.nu - any changes here will be overwritten. -->\n"
    print "# General"
    print $formatted_mdata.preamble
...
def get_license_texts [dep: record] {
    ...
    if (($dep.license_files | length) < 1) {
        print $"Error: ($dep.name) does not have any license files"
        1 / 0
    }
...
def get_local_src_path [cargo_dep: record] {
    ...
    if ($depended_targets | is-empty) {
        print $"Error: The depended target for ($cargo_dep.name) could not be determined as there were multiple viable targets"
        exit 1
    } else if ($depended_targets | length) > 1 {
        print $"Error: No viable targets were found for ($cargo_dep.name)"
        exit 1
    }
...
def to_md [] {
    let dep = $in
    print $"## ($dep.name)"
    print $"The `($dep.name)` software is included in this product."
    print $"The source code is available here: ($dep.repo)."
    print "Its license(s) and notice(s) are as follows:\n"
    print $dep.license_txt
}
