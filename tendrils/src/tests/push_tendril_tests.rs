...
fn local_is_symlink_returns_type_mismatch_error_unless_forced_then_copies_symlink_target_contents_keeps_local_name(
    ...
    let file_actual =
        push_tendril(&setup.td_dir, &file_tendril, dry_run, force);
    let dir_actual = push_tendril(&setup.td_dir, &dir_tendril, dry_run, force);
...
fn no_read_access_from_local_file_returns_io_error_permission_denied_unless_dry_run(
    ...
    let actual = push_tendril(&td_dir, &tendril, dry_run, force);
    ...
    assert_eq!(
        ...
        ActionLog::new(
            Some(FsoType::File),
            None,
            temp_parent_dir.path().join("no_read_access.txt"),
            exp_result,
        )
    );
...
fn no_read_access_from_local_dir_returns_io_error_permission_denied_unless_dry_run(
    ...
    assert_eq!(
        ...
        ActionLog::new(
            Some(FsoType::Dir),
            None,
            temp_parent_dir.path().join("no_read_access_dir"),
            exp_result,
        )
    );
    assert!(is_empty(temp_parent_dir.path()));
